				...
				<form method="post" action="nearbyJobs" id="resubmitJobs">
					<div class="row address-box title container">
						...
						<ul class="dropdown-menu secondary-text">
							<li><p>
									<select class="industry_dropdown" name="industry">
										<option value="0"
		...
		<div class="container">
			<div class="content" id="load-list">
				<c:forEach var="job" items="${jobs}">
					<c:url value="/viewJob/${job.metadata}/" var="url" />
					<div class="row object" onclick="window.location.pathname='${url}'">
						<div class="item">
							...
							<div class="row minor-text">
								<c:out value='${job.tagline}' />
							</div>
							<div class="row minor-text subheading">
								<div class="interest">
									<c:out value='${job.industry}' />
									&nbsp;-&nbsp;
				...
				<c:if test="${not empty address}">
					<c:if test="${fn:length(jobs) lt 20}">
						<div class="row object banner primary-text">
							No more jobs in that area. <br /> We are still growing! <a
								href="#" onclick="$('#input-address').focus();">Try another
								area?</a>
						</div>
					</c:if>
				</c:if>
				<c:if test="${not empty jobs}">
					<div class="row object">
						...
						<c:if test="${fn:length(jobs) gt 19}">
							<div class="next-page btn btn-default"
								onclick="$('#jobs_form').submit();">
		...
		<div class="container">
			...
			<div class="navbar-collapse collapse">
				<ul class="nav navbar-nav navbar">
					...
					<li><a href="#" onclick="switchToSeekers();">SWITCH TO
							NEARBY USERS</a></li>
					<li><a href="#"
